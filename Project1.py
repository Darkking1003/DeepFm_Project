...
with tf.name_scope("Batch"):
    ...
    Dataset = tf.data.experimental.CsvDataset(
        "C:/Users/musta/OneDrive/Masaüstü/MovieLens/ml-latest-small/Processed/Training_data.csv", record_defaults)
    ...
    test_set = tf.data.experimental.CsvDataset(
        "C:/Users/musta/OneDrive/Masaüstü/MovieLens/ml-latest-small/Processed/Test_data.csv", record_defaults)
    ...
    with tf.name_scope("Deep"):
        ...
        hidden1 = tf.layers.dense(Deep_Input, 320, activation=tf.nn.leaky_relu,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(0.01))
        h1_drop = tf.layers.dropout(hidden1, 0.2, training=Training)
        hidden2 = tf.layers.dense(h1_drop, 320, activation=tf.nn.leaky_relu,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(0.01))
        h2_drop = tf.layers.dropout(hidden2, rate=0.2, training=Training)
        hidden3 = tf.layers.dense(hidden2, 320, activation=tf.nn.leaky_relu,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(0.01))
        ...
        for epoch in range(Epoch_num):
            ...
            try:
                ...
                while True:
                    ...
                    Out = sess.run(training_op,
                                   feed_dict={X: User_Movie[Batch[0]], X1: User_Movie[:, Batch[1]].T, Y: labels})
                    if i == 0:
                        Deneme = sess.run(w,
                                          feed_dict={X: User_Movie[Batch[0]], X1: User_Movie[:, Batch[1]].T, Y: labels})
                        print(Deneme)
